...
Create Combine publishers:

```swift
extension AuthSession {
    static var publisher: AnyPublisher<AuthSession?, Never> {
        SharedReader(.authSession).publisher
    }
}

extension JWTAuthClient {
    func sendAuthenticatedPublisher<T: Decodable>(
        _ request: Request
    ) -> AnyPublisher<SuccessResponse<T>, Error> {
        Future { promise in
            Task {
                do {
                    let response: SuccessResponse<T> = try await self.sendAuthenticated(request)
